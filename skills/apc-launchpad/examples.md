...
8. User: App Privacy practices + listing screenshots; `eas credentials` → **`pnpm build:ios`** (local). Upload with ASC `ipa:upload` (never `eas submit`).
...
3. Build: if EAS says credentials not set up → user runs `eas credentials -p ios`, then agent **`pnpm build:ios`** (local). Do not cloud-build. Upload IPA with ASC `ipa:upload` (never `eas submit`).
